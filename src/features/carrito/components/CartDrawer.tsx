    ...
    return (
        <>
            ...
            <div style={{
                ...
                width: "min(420px,100vw)", background: "#F2E8D5", zIndex: 40,
                display: "flex", flexDirection: "column",
                boxShadow: "-8px 0 40px rgba(0,0,0,0.15)",
                animation: "slideIn 0.3s ease",
            }}>
                {/* Header */}
                <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #e8ddd5", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                    ...
                    {items.length === 0 ? (
                        <div style={{ textAlign: "center", padding: "60px 0", color: "#9a8070" }}>
                            <div style={{ marginBottom: 12, display: "flex", justifyContent: "center" }}>
                                <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="#2d1e0f" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.6 }}>
                            ...
                            {items.map((item) => (
                                <div key={item.producto_id} style={{ display: "flex", gap: 12, background: "#F2E8D5", borderRadius: 16, padding: 12 }}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <p style={{ margin: "0 0 8px", fontSize: 13, color: "#2d1e0f", fontWeight: 700 }}>${item.precio.toFixed(2)} c/u</p>
                ...
                {/* Footer */}
                {items.length > 0 && (
                    <div style={{ padding: "16px 24px 24px", borderTop: "1px solid #e8ddd5" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 18, fontWeight: 800, color: "#2d1e0f", marginBottom: 16 }}>
                            <span>Total</span>
                            <span style={{ color: "#2d1e0f" }}>${totalFinal.toFixed(2)}</span>
                        </div>
                        <button
                            onClick={() => { onClose(); navigate("/realizar-pedido"); }}
                            style={{ width: "100%", background: "#2d1e0f", color: "#F2E8D5", border: "none", borderRadius: 16, padding: "16px", fontSize: 14, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer" }}
                        >
                            Confirmar Pedido →
                        </button>
                    </div>
