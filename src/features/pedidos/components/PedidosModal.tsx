...
export function PedidosModal({ open, onClose }: Props) {
  ...
  if (!user) {
    return (
      <>
        ...
        <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 60, background: "#fff", borderRadius: 24, padding: 40, textAlign: "center", width: "min(320px, 90vw)", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
          <ReceiptIcon width={48} height={48} style={{ color: "#c8722a", opacity: 0.4, marginBottom: 16 }} />
          <p style={{ color: "#9a8070", marginBottom: 20, fontWeight: 600 }}>Iniciá sesión para ver tus pedidos</p>
          <button onClick={() => { onClose(); navigate("/login"); }} style={{ background: "#c8722a", color: "#fff", border: "none", borderRadius: 12, padding: "12px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
            Iniciar sesión
          </button>
        </div>
      </>
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #f0e8e0", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px 24px" }}>
          {isLoading ? (
            ...
            <div style={{ textAlign: "center", padding: "40px 0", color: "#9a8070" }}>
              <ReceiptIcon width={48} height={48} style={{ color: "#c8722a", opacity: 0.3, marginBottom: 16 }} />
              <p style={{ fontWeight: 600, margin: "0 0 20px" }}>No tenés pedidos todavía</p>
              <button
                onClick={() => { onClose(); navigate("/"); }}
                style={{ background: "#c8722a", color: "#fff", border: "none", borderRadius: 12, padding: "12px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}
              >
                Ver menú
              </button>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {pedidos.map((p) => (
                <div key={p.id} style={{ background: "#fdf9f6", borderRadius: 16, padding: 16, border: "1px solid #f0e8e0" }}>
                  {/* Header */}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, borderTop: "1px solid #e8ddd5", paddingTop: 10, marginTop: 8 }}>
                    ...
                    <span style={{ fontSize: 14, color: "#c8722a" }}>${toNumber(p.total).toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
